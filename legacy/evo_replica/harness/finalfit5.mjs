...
// ---- collect all usable toric cases over the EXTENDED domain --------------
const DS = [];
for (const [k, v] of Object.entries(C)) {
  ...
  if (!isFinite(fn.P0) || fn.P0 <= 6) continue;              // stay on the validated branch
...
const nLong = DS.filter(d => d.ALeff > 27.5).length;
console.log(`cases: ${DS.length} (fit ${FIT.length} / holdout ${HOLD.length});  long eyes AL>27.5: ${nLong}`);

// ---- model forms -----------------------------------------------------------
const FORMS = {
  lin:    { n: 4, post: (p, d) => p[1] + p[2] * (d.Km - 44) + p[3] * (d.ALeff - 23.5) },
  quadA:  { n: 5, post: (p, d) => p[1] + p[2] * (d.Km - 44) + p[3] * (d.ALeff - 23.5) + p[4] * (d.ALeff - 23.5) ** 2 },
  quadAK: { n: 6, post: (p, d) => p[1] + p[2] * (d.Km - 44) + p[3] * (d.ALeff - 23.5) + p[4] * (d.ALeff - 23.5) ** 2 + p[5] * (d.Km - 44) * (d.ALeff - 23.5) },
  quad2:  { n: 7, post: (p, d) => p[1] + p[2] * (d.Km - 44) + p[3] * (d.ALeff - 23.5) + p[4] * (d.ALeff - 23.5) ** 2 + p[5] * (d.Km - 44) * (d.ALeff - 23.5) + p[6] * (d.Km - 44) ** 2 },
};

function score(form, p, set) {
  let sse = 0, n = 0, mx = 0, axSum = 0, axN = 0;
  for (const d of set) {
    const post = form.post(p, d);
    let t = [p[0] * d.ant[0] + post, p[0] * d.ant[1]];
...
function fit(form, seed, steps) {
  let p = seed.slice();
  for (let r = 0; r < 30; r++) for (let i = 0; i < form.n; i++) {
    let lo = p[i] - steps[i], hi = p[i] + steps[i];
    for (let it = 0; it < 36; it++) {
      const m1 = lo + (hi - lo) / 3, m2 = hi - (hi - lo) / 3;
      const a = p.slice(); a[i] = m1; const b = p.slice(); b[i] = m2;
      if (score(form, a, FIT).rms < score(form, b, FIT).rms) hi = m2; else lo = m1;
    }
    p[i] = (lo + hi) / 2;
  }
  return p;
...
for (const [name, form] of Object.entries(FORMS)) {
  const seed = [0.98, 0.61, -0.031, -0.063, 0, 0, 0].slice(0, form.n);
  const steps = [0.08, 0.15, 0.03, 0.04, 0.02, 0.01, 0.01].slice(0, form.n);
  const p = fit(form, seed, steps);
  const f = score(form, p, FIT), h = score(form, p, HOLD);
  const hLong = score(form, p, HOLD.filter(d => d.ALeff > 27.5));
  results[name] = { p, h };
  console.log(`${name.padEnd(7)} [${p.map(x => x.toFixed(5)).join(', ')}]`);
  console.log(`         fit rms ${f.rms.toFixed(4)} ax ${f.axErr.toFixed(2)}°   HOLDOUT rms ${h.rms.toFixed(4)} max ${h.mx.toFixed(3)} ax ${h.axErr.toFixed(2)}°   holdout-largo rms ${hLong.rms.toFixed(4)} ax ${hLong.axErr.toFixed(2)}° (n=${hLong.n})`);
}
let best = null;
for (const [name, r] of Object.entries(results)) if (!best || r.h.rms < results[best].h.rms) best = name;
console.log('\nbest form by holdout:', best);
fs.writeFileSync('toricmodel5.json', JSON.stringify({ form: best, p: results[best].p }, null, 1));
console.log('wrote toricmodel5.json');
