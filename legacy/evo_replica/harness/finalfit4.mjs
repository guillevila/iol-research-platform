...
for (const [k, v] of Object.entries(C)) {
  ...
  if (!(ALeff >= 20.5 && ALeff <= 31.5 && Km >= 34.5 && Km <= 50)) continue;
  { const t = eyeFn(ALeff, Km, Aeff); if (!(t.P0 > 6)) continue; }
...
function score(p, set) {
  const [sa, p0, pK, pAL] = p;
  let sse = 0, n = 0, mx = 0, axSum = 0, axN = 0;
  for (const d of set) {
    const post = p0 + pK * (d.Km - 44) + pAL * (d.ALeff - 23.5);
...
let p = [1.0, 0.58, -0.015, -0.04];
const steps = [0.1, 0.2, 0.03, 0.05];
...
fs.writeFileSync('toricmodel4.json', JSON.stringify({ sa: p[0], p0: p[1], pK: p[2], pAL: p[3] }, null, 1));
